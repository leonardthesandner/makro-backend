...
const FRONTEND_URL = process.env.FRONTEND_URL || "https://leonardthesandner.github.io/makro";
...
async function sendPasswordResetEmail(email, token) {
  const url = `${FRONTEND_URL}?reset=${token}`;
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: "Passwort zurücksetzen – makro.",
    html: `
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#0c0c0c;font-family:sans-serif;">
  <div style="max-width:480px;margin:0 auto;padding:48px 24px;">
    ...
    <div style="color:#555;font-size:13px;margin-bottom:40px;">Dein Makro-Tracker</div>
    <div style="color:#efefef;font-size:18px;font-weight:700;margin-bottom:10px;">Passwort zurücksetzen</div>
    <div style="color:#888;font-size:14px;line-height:1.7;margin-bottom:32px;">
      Klick auf den Button um ein neues Passwort zu vergeben. Der Link ist 1 Stunde gültig.
    </div>
    <a href="${url}"
       style="display:inline-block;background:#f59e0b;color:#000;font-weight:700;font-size:15px;padding:14px 32px;border-radius:8px;text-decoration:none;">
